...
export function getJewelryPlacement(
  ...
  switch (jewelryType) {
    ...
    case 'NOSE-PIN': {
      ...
      const targetWidth = noseLength * 0.4;
      const targetHeight = targetWidth / imageAspectRatio;

      return {
        width: targetWidth,
        height: targetHeight,
        x: nostril.x * canvas.width - targetWidth / 2,
        y: nostril.y * canvas.height - targetHeight / 2,
      };
    }
    ...
    case 'BRACELET': {
      ...
      const scaleMultiplier = 1.8; // Increased from 1.2, adjust as needed
    ...
    case 'HIPBELT': {
      ...
      const verticalOffset = -hipWidth * 0.8; // <-- ADJUST THIS VALUE
    ...
    case 'FOREHEAD': {
      ...
      const targetWidth = foreheadWidth * 2.2;
      const targetHeight = targetWidth / imageAspectRatio;

...
// Assuming sonner is correctly imported and configured in your project
import { toast } from "sonner"; // Use default import if configured that way
...
function Header() {
  return (
    <div className="bg-[#f6e1d2] relative shrink-0 w-full" data-name="Header">
    ...
    <div className="relative shrink-0 w-full" data-name="Breadcrumbs">
      <div className="overflow-clip rounded-[inherit] size-full">
        ...
        <div className="box-border content-stretch flex gap-2 md:gap-[10px] items-center px-4 sm:px-8 md:px-12 lg:px-16 xl:px-24 py-4 sm:py-6 md:py-8 lg:py-12 relative w-full">
          ...
          <button
            onClick={onBack} // Call the passed-in onBack function when clicked
            ...
            style={{ background: 'transparent', border: 'none', padding: 0 }} // Basic button styling
...
function FormField({ value, onChange, placeholder, type = "text" }: FormFieldProps) {
  return (
    // Responsive rounding
...
function Frame222({ value, onChange }: { value: string; onChange: (value: string) => void }) {
  return (
    // Responsive gap
...
function Frame223({ value, onChange }: { value: string; onChange: (value: string) => void }) {
  return (
    // Responsive gap
    <div className="content-stretch flex flex-col gap-2 md:gap-3 items-start relative shrink-0 w-full">
      <Frame220 />
...
function Frame224({ value, onChange }: { value: string; onChange: (value: string) => void }) {
  return (
    // Responsive gap
    <div className="content-stretch flex flex-col gap-2 md:gap-3 items-start relative shrink-0 w-full">
      <Frame221 />
...
function LanguageButton({ label, isSelected, onClick }: LanguageButtonProps) {
  return (
    <button
      type="button" // Prevent form submission
      onClick={onClick}
      // Styling changes based on isSelected state, added hover state
      className={`basis-0 grow min-h-px min-w-px relative rounded-lg md:rounded-[12px] shrink-0 transition-colors ${
        isSelected ? "bg-[#e29d6f]" : "bg-[#fcf5f1] hover:bg-[#f6e1d2]"
      }`}
    ...
    >
      <div className="flex flex-row items-center justify-center overflow-clip rounded-[inherit] size-full">
        ...
        <div className="box-border content-stretch flex gap-1 md:gap-[10px] items-center justify-center px-2 sm:px-4 md:px-6 lg:px-8 py-2 sm:py-3 md:py-4 relative w-full">
          ...
          <div className={`flex flex-col font-['Poppins:Regular',_sans-serif] justify-center leading-[0] not-italic relative shrink-0 text-xs sm:text-sm md:text-base lg:text-lg xl:text-[24px] text-nowrap ${
            isSelected ? "text-[#fcf5f1]" : "text-[#ce8f65]"
          }`}>
      ...
      <div aria-hidden="true" className={`absolute border border-solid inset-0 pointer-events-none rounded-lg md:rounded-[12px] ${
        isSelected ? "border-[#ce8f65]" : "border-[#e29d6f]"
      }`} />
    </button>
  );
}
...
function Frame226({ selectedLanguage, onLanguageChange }: { selectedLanguage: string; onLanguageChange: (lang: string) => void }) {
  return (
    // Responsive gap
...
function Frame225({ selectedLanguage, onLanguageChange }: { selectedLanguage: string; onLanguageChange: (lang: string) => void }) {
  return (
     // Responsive gap
    <div className="content-stretch flex flex-col gap-2 md:gap-3 items-start relative shrink-0 w-full">
      <Frame238 />
...
function TimeInput({ value, onChange, options, placeholder }: TimeInputProps) {
  const [showDropdown, setShowDropdown] = useState(false);

  return (
    <div className="relative content-stretch flex gap-1 md:gap-2 items-start shrink-0">
      ...
      <div className="bg-[#fcf5f1] relative rounded-lg md:rounded-[12px] shrink-0 min-w-[60px] sm:min-w-[80px] md:min-w-[100px]" data-name="Form field">
        ...
        <div className="box-border content-stretch flex gap-1 md:gap-[10px] items-center justify-center overflow-clip px-1 sm:px-2 md:px-3 py-1 sm:py-2 md:py-2.5 relative rounded-[inherit]">
          <input
            ...
            className="flex flex-col font-['Poppins:Regular',_sans-serif] justify-center leading-[0] not-italic relative shrink-0 text-[#7c563d] text-xs sm:text-sm md:text-base lg:text-lg xl:text-[24px] w-8 sm:w-10 md:w-12 bg-transparent outline-none text-center placeholder:text-[#ce8f65]"
            readOnly // Make input read-only if options are provided to force selection
          ...
          {options && (
            <button
              ...
              className="flex size-6 sm:size-7 md:size-8 items-center justify-center relative shrink-0"
              style={{ "--transform-inner-width": "47.984375", "--transform-inner-height": "47.984375" } as React.CSSProperties} // Keep style if needed for complex transforms
...
function Frame241({ time, onTimeChange }: { time: { hour: string; minute: string; period: string }; onTimeChange: (field: 'hour' | 'minute' | 'period', value: string) => void }) {
  ...
  return (
    ...
    <div className="content-stretch flex gap-2 sm:gap-3 md:gap-4 items-start relative shrink-0">
      <TimeInput
        value={time.hour}
        onChange={(val) => onTimeChange('hour', val)}
        options={hours}
        placeholder="HH"
      />
      <TimeInput
        value={time.minute}
        onChange={(val) => onTimeChange('minute', val)}
        options={minutes}
        placeholder="MM"
      />
      <TimeInput
        value={time.period}
        onChange={(val) => onTimeChange('period', val)}
        options={periods}
        placeholder="AM/PM" // Changed placeholder
      />
    </div>
  );
}
...
function Frame242({ time, onTimeChange }: { time: { hour: string; minute: string; period: string }; onTimeChange: (field: 'hour' | 'minute' | 'period', value: string) => void }) {
  return (
    // Responsive gap
    <div className="content-stretch flex flex-col gap-2 md:gap-3 items-start relative shrink-0 w-full">
      <Frame239 />
...
function FormField9({ onClick }: { onClick?: () => void }) { // Made onClick optional as it triggers form submit
  return (
    <button
      onClick={onClick} // Keep onClick if needed for other actions, but submit is handled by form
      // Added hover state
      className="bg-[#7c563d] relative rounded-lg md:rounded-[12px] shrink-0 w-full hover:bg-[#5f422f] transition-colors"
      data-name="Form field"
...
function Frame243({ onSubmit }: { onSubmit: () => void }) { // onSubmit might not be strictly needed here if handled by form
  return (
...
function InfoContainer({ formData, onFormChange, onSubmit }: InfoContainerProps) {
  return (
     // Added <form> element for semantic structure and form handling
    // Responsive gap and width
    <form
      onSubmit={onSubmit} // Handle form submission here
      ...
      noValidate // Prevent default browser validation, rely on custom validation
      >
      ...
      <Frame242 time={formData.time} onTimeChange={(field, val) => onFormChange('time', { ...formData.time, [field]: val })} />
...
function ProductContainer({ formData, onFormChange, onSubmit }: ProductContainerProps) {
  return (
    <div className="relative shrink-0 w-full" data-name="Product Container">
...
function Frame235() {
  return (
    <div className="basis-0 content-stretch flex gap-2 md:gap-[10px] grow items-center justify-center min-h-px min-w-px relative shrink-0">
       {/* Responsive text size */}
...
interface ProductPageProps {
  onBack?: () => void; // Prop for handling the back navigation
}
...
export function ProductPage({ onBack }: ProductPageProps) {
  ...
  // CHANGE: Changed parameter type to React.FormEvent<HTMLFormElement>
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    event.preventDefault(); // CHANGE: Prevent default browser form submission
    if (!validateForm()) {
      return; // Stop submission if validation fails
    }

    // Prepare data for submission (e.g., to an API or database)
    ...
    toast(
      <div className="flex items-center gap-3">
        <div className="h-10 w-10 rounded-full bg-[#7c563d] flex items-center justify-center shrink-0">
          <svg className="h-6 w-6 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 13l4 4L19 7" />
          </svg>
        </div>
        <div>
          <p className="font-['Playfair_Display:Medium',_sans-serif] text-base font-medium text-[#5f422f]">
            Booking Successful!
          </p>
          <p className="mt-0.5 font-['Poppins:Regular',_sans-serif] text-sm text-[#7c563d]">
      ...
      </div>,
      {
        duration: 5000,
        position: "top-center",
        className: "bg-white border border-[#e29d6f] shadow-lg",
      }
    );
